...
| **Channel message received** | A character sharing the sender's zone hears a message in an allowed channel (General by default, see `PBC.ChannelNamePrefixes`). Rolls work the same as chat messages, but use the separate (lower-default) `ReplyChanceChannelMessage`/`ReplyChanceChannelMention` knobs, and the candidate pool is capped by `PBC.ChannelMessageMaxCandidates`. Replies are sent back into the same channel. | `John says: Anyone selling a Thunderfury?` |
